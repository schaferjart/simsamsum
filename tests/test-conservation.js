...
console.log('TEST: Testing Conservation of Mass Fix');
...
if (window.workflowApp) {
    ...
    console.log('SEARCH: Incoming connections to pre_video_mail:');
    ...
    console.log(`\nSTATS: pre_video_mail variable: ${preVideoNode?.variable}`);
    
    console.log('\nSYNC: Triggering calculation...');
    ...
    setTimeout(() => {
        ...
        console.log('\nTARGET: Expected for pre_video_mail:');
        ...
        console.log(`\nFLOW: Actual pre_video_mail result: ${actualPreVideo?.incomingNumber}`);
        
        if (actualPreVideo?.incomingNumber == 2000) {
            console.log(' SUCCESS! Conservation of mass working correctly.');
        } else {
            console.log('ERROR: Still not correct. Need to investigate further.');
        }
    }, 1000);
